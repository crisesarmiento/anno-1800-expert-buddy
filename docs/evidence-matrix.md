...
| `quests[]` | Vigilante: siempre `[]`. Parser TS: GUIDs de quest **sin estado**. JSON: títulos; `state` opcional; opcionales `instanceId`, `guid`, `type`, `objectives`, `progress`, `timer` si vienen decodificados | Instancia si hay `instanceId` (repite GUID). Sin estado, el GUID no es activo ni save-read | título + estado declarado; progreso/temporizador congelados en el save | `savedAt` si save-read | Watcher: lista vacía a propósito. Save-read: solo con estado explícito. Manual: checklist rotulada | no disponible (watcher); parcial (JSON explícito); validado (semántica: GUID sin estado ≠ activa, ausencia ≠ hecha, edificio ≠ completa) |
...
| `telemetry.routes[].delivery` | `PassiveTrade/History/TradeRouteEntries` (finalizadas) | Ruta; `GoodAmount` con signo respecto de la isla que guardó la visita | visitas, mediana \|cantidad\|, último amount, intervalo de `ExecutionTime`; `stations[]` por `areaId`+bien cuando el save trae estación | `simTime` / `ExecutionTime` | Si hay visitas finalizadas | validado (visitas). **inferido** el t/min por destino/bien. Sin `areaId` no hay tasa de destino. no es capacidad nominal ni abastecimiento garantizado |
| Throughput de ruta | Derivado opcional en UI (`stations[].medianAbsAmount / stations[].interval`) | Destino + bien, no la ruta mezclada | t/min inferidos | reloj de simulación | Sólo con visitas, estación y intervalo de esa serie | inferido; Taller no lo usa como transporte real. Una carga/descarga alternada no es el ritmo del destino |
