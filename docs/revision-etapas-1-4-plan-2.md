...
## P2-C misiones y flota (2026-09-20)

Estado: **hecho parcial**. Un solo PR, solo escritorio; extiende `/taller` → Flota y el panel de misiones existentes, sin reescritura ni cambios de contrato de telemetría.

| Ítem del encargo | Estado | Evidencia / límite |
| --- | --- | --- |
| 1. Objetivos y reservas con evidencia; estado manual rotulado | **hecho** | `SuggestedReserve` ahora lleva `amountHonesty: "confirmed"` (el requisito sale directo del objetivo del save) y `stockHonesty` (`"confirmed"` solo si `telemetry.goods` trajo ese bien). La UI de reservas (`mission-channels.tsx`, panel y tarjeta de Taller) muestra esa etiqueta junto al stock. Cada misión leída del save ahora también lista sus objetivos (`current/required` + nombre del bien) marcados "Confirmado". El canal manual ya distinguía su rótulo (`Confirmación manual` / `Sin estado validado`); no se tocó esa lógica porque ya cumplía. |
| 2. Inventario de cobertura explícito | **hecho** | `fleetRoleInventory` (nuevo, en `src/lib/fleet/coverage.ts`) cuenta cada casco leído bajo exactamente un rol manual (escolta/defensa/comercio/sin asignar/sin función) + total. `FleetAdvice.roleInventory` lo expone; `FleetViewCard` lo muestra como "Inventario de cobertura" antes de la lista de barcos. No reemplaza el aviso de riesgo no evaluable (diplomacia/defensas/escolta siguen "missing") — es un inventario de qué cubre la función manual, no una evaluación militar. |
| 3. Subtotal de mantenimiento conocido nada más | **hecho** | `fleetUpkeepSplit` ya excluía barcos sin cifra de catálogo del total; ahora la UI lo dice explícitamente: cuando `withoutUpkeep > 0` aparece "Subtotal de mantenimiento conocido nada más: N de M barcos sin cifra de catálogo" debajo del desglose comercio/militar/total. Nunca inventa upkeep de casco sin dato de catálogo. |
| 4. Separar compra / construcción / sostenimiento | **hecho** | El comparador de casco nuevo en Flota se dividió en tres bloques rotulados: **Compra** (precio de lista vs. caja), **Construcción** (materiales vs. stock, con isla), **Sostenimiento** (mantenimiento de catálogo inferido, ahora visible — antes `compare.upkeep` se calculaba pero no se mostraba en absoluto). |
| 5. Materiales en la isla/astillero correspondiente | **hecho** (cuando el save trae `islandSnapshots[].stock`) / **límite explícito** en el resto | `compareNewBuild(shipId, snapshot, islandId?)` acepta ahora una isla. Con isla elegida, el stock sale de `islandSnapshots[].stock` de esa colonia (`materialsScope: "island"`) y la UI dice "Stock leído en {isla}". Sin isla identificable coincidente, cae a `"unknown"` y lo declara ("No identifiqué una isla de jugador..."). Solo cuando no hay ninguna isla elegida cae al stock global del save (`"global"`) y lo etiqueta como tal — nunca presenta stock global como si fuera de una isla puntual. `FleetViewCard` agrega un selector de isla/astillero (mismo patrón que el selector de Simulador) que aparece solo si `islandSnapshots` trajo colonias del jugador. |
| 6. Nunca prometer seguridad o resultado de combate | **hecho, sin cambios** | `recommendDismantleDefense`/`promiseCombatOutcome` seguían siempre `false`; ya cubierto por P1-A/etapa 6, no se tocó. |

Regresión: 2 pruebas nuevas en `fleet.test.ts` (inventario de cobertura por rol; materiales por isla vs. global vs. isla desconocida) y 1 en `missions.test.ts` (evidencia de reserva confirmada vs. stock no leído). `npm test`: 75 archivos app / 644 pruebas, 15 archivos scripts / 83 pruebas, todas en verde. `npm run typecheck`, `npm run lint` (0 errores, solo warnings preexistentes no relacionados) y `npm run build` en verde.

Qué se puede confiar: una reserva de misión nunca muestra "confirmado" sin salir del save; el inventario de cobertura cuenta exactamente los barcos leídos, nunca infiere seguridad de la ausencia de enemigos; el subtotal de mantenimiento nunca oculta cuántos barcos quedan fuera del cálculo; el bloque de Construcción nunca llama stock de una isla al stock global del save.

Límites / pendiente:
- El selector de isla del comparador de flota depende de que el save traiga `islandSnapshots[].stock` (gap ya documentado en P1-B/AGENTS.project.md); sin eso, el comparador sigue funcionando pero con stock global rotulado como tal, no como límite nuevo de este PR.
- No hay lectura real de astillero (qué isla puede construir qué casco, cola de construcción, tiempo de build): el "Construcción" de este PR es materiales + isla elegida manualmente, no una cola de astillero leída del save.
- `ConstructionAI ShipMaintenance` sigue sin publicarse (sin dueño verificado); el mantenimiento sigue siendo inferido de wiki, nunca el ticker del juego — sin cambios respecto a etapa 6/P1-A.
- No se tocó P3 (modificadores, DLC, Docklands, escenarios conjuntos).
