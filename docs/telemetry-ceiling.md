...
`pulseHint` (coins / houses) es opcional. El vigilante lo calcula desde señales reales del escaneo: `coins` del delta de dinero **del participante 0** contra el escaneo anterior, `houses` de la presencia de residencias / mercado / pescadería. Sin dueño player o sin señal clara queda `unknown` — **nunca adivina** `down`/`empty` ni toma el máximo de tesorerías ajenas. Campos: `docs/harbor-live-fields.md`, `docs/evidence-matrix.md`. Schema: `docs/harbor-live.schema.json`.
