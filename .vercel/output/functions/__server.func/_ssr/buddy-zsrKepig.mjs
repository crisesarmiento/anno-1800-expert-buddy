...
import { a as defaultPulse, c as lifeByChapter, f as peopleForChapter, i as chaptersById, n as buildingsById, p as pulseLine, t as brokeSteps, u as missionsById } from "./play-CGDHSrxw.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/buddy-zsrKepig.js
...
function plainTalk(text) {
	return text.replace(/\*\*(.*?)\*\*/g, "$1").replace(/__(.*?)__/g, "$1").replace(/(^|[^\w])\*(.*?)\*(?!\w)/g, "$1$2").replace(/^#{1,6}\s+/gm, "").replace(/^\s*[-*]\s+/gm, "• ").trim();
}
function missionContext(missionId, spoilers, pulse, checked) {
	if (!missionId) return "Todavía no eligió misión. Ayudalo a ubicar dónde está. Si recién funda, Capítulo 1, Una chispa reavivada.";
	...
	const buildingLines = mission.buildingIds.map((id) => buildingsById[id]).filter(Boolean).map((building) => `- ${building.name}: ${building.buddy}`).join("\n");
	const life = lifeByChapter[mission.chapterId];
	const folks = peopleForChapter(mission.chapterId).map((person) => `- ${person.name} (${person.role}): ${person.buddy} HACÉ: ${person.do} NO: ${person.dont}`).join("\n");
	...
	const spoilerLine = spoilers && mission.spoilers ? `Spoilers permitidos: ${mission.spoilers}` : "Spoilers APAGADOS. No cuentes giros, traiciones ni el final.";
	return [
		pulseLine(pulse),
		...
		life ? `Plata de este capítulo: ${life.money.pulse}\nPara que suban las monedas: ${life.money.keepGreen.join(" | ")}\nTrampa de plata: ${life.money.trap}` : "",
		`Si las monedas están en rojo: ${brokeSteps.join(" | ")}`,
		life ? `Diplomacia: ${life.diplomacy.pulse}\nPaz: ${life.diplomacy.keepPeace.join(" | ")}\nTrampa diplomática: ${life.diplomacy.trap}` : "",
		folks ? `Gente:\n${folks}` : "",
		spoilerLine
	].filter(Boolean).join("\n");
}
var askBuddy_createServerFn_handler = createServerRpc({
	id: "f345c695ab2916dd3456ae42ecfc6f01fcc27c17e5b4d82584ab007059211537",
	name: "askBuddy",
	filename: "src/lib/buddy.ts"
}, (opts) => askBuddy.__executeServer(opts));
var askBuddy = createServerFn({ method: "POST" }).validator((input) => {
...
}).handler(askBuddy_createServerFn_handler, async ({ data }) => {
	...
	const messages = [
		{
			role: "system",
			content: `Sos Harbor Buddy, sentado al lado de un amigo que juega la campaña de Anno 1800 en Windows, segundo monitor. Hablás español rioplatense (vos, tenés, podés). Nunca vosotros. Nunca un tono de España.
...
${missionContext(data.missionId, data.spoilers, data.pulse ?? defaultPulse, data.checked ?? [])}`
	...
	try {
		...
		if (!res.ok) return {
			ok: false,
			error: "No llegó la radio del puerto."
		};
		const text = plainTalk((await res.json()).choices?.[0]?.message?.content?.trim() ?? "");
		if (!text) return {
			ok: false,
			error: "Se quedó mirando el agua. Probá otra vez."
		};
		return {
			ok: true,
			text
		...
		return {
			ok: false,
			error: "Estática. Preguntá de nuevo en un rato."
		};
	}
});
//#endregion
export { askBuddy_createServerFn_handler };
